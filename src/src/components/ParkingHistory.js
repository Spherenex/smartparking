...
import { rtdb } from '../firebase';
import { ref, onValue, query, orderByChild, equalTo, update, get, set, push, serverTimestamp } from 'firebase/database';
import { Calendar, Clock, MapPin, Activity, ChevronRight, CreditCard, Banknote, CheckCircle, Camera, Check, X } from 'lucide-react';
import '../styles/ParkingHistory.css';

// SlotCard Component for Live Camera Simulation
const SlotCard = ({ slotNumber, slotData }) => {
  const isOccupied = determineSlotOccupancy(slotNumber, slotData);
  
  // Get sensor values from slot data
  const getSensorValues = () => {
    if (!slotData) return { sensor1: null, sensor2: null, sensor1Triggered: false, sensor2Triggered: false };
    
    return {
      sensor1: slotData.sensor1_value || 0,
      sensor2: slotData.sensor2_value || 0,
      sensor1Triggered: slotData.sensor1_triggered || false,
      sensor2Triggered: slotData.sensor2_triggered || false
    };
  };
  
  const sensorData = getSensorValues();
  
  return (
    <div className={`slot-card ${isOccupied ? 'occupied' : 'vacant'}`}>
      <div className="slot-header">
        <h3>Slot {slotNumber}</h3>
        <div className={`slot-status ${isOccupied ? 'status-occupied' : 'status-vacant'}`}>
          {isOccupied ? (
            <>
              <X size={16} />
              <span>Occupied</span>
      ...
      <div className="sensor-data">
        <div className="sensor-group">
          ...
          <div className="sensor-value">
            Value: {sensorData.sensor1}
            <span className={`sensor-status ${sensorData.sensor1Triggered ? 'triggered' : ''}`}>
              {sensorData.sensor1Triggered ? 'Triggered' : 'Not Triggered'}
            </span>
          </div>
        </div>
        ...
        <div className="sensor-group">
          ...
          <div className="sensor-value">
            Value: {sensorData.sensor2}
            <span className={`sensor-status ${sensorData.sensor2Triggered ? 'triggered' : ''}`}>
              {sensorData.sensor2Triggered ? 'Triggered' : 'Not Triggered'}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="slot-footer">
        <div className="occupancy-logic">
          Occupancy Logic: {isOccupied ? 'Sensors match pattern for this slot' : 'Waiting for sensor pattern match'}
        </div>
      </div>
    </div>
  );
...
// Function to determine slot occupancy based on sensor numbers
const determineSlotOccupancy = (slotNumber, slotData) => {
  if (!slotData) return false;
  
  // Check if slot is explicitly marked as occupied
  if (slotData.occupied === true) return true;
  
  // Apply the occupancy logic based on sensor numbers
  if (slotNumber === 1) {
    return slotData.sensor1_number === 1 && slotData.sensor2_number === 2;
  } else if (slotNumber === 2) {
    return slotData.sensor1_number === 3 && slotData.sensor2_number === 4;
  } else if (slotNumber === 3) {
    return slotData.sensor1_number === 4 && slotData.sensor2_number === 5;
  }
...
const ParkingHistory = ({ userId, onBookingsFetched }) => {
  ...
  // States for Live Camera Simulation
  ...
  useEffect(() => {
    ...
    // Fetch slot data when camera simulation is shown
    if (showCameraSimulation) {
      fetchSlotsData();
    }
  }, [userId, filter, showCameraSimulation]);

  const fetchSlotsData = () => {
    setLoadingSlots(true);
    try {
      const parkingDataRef = ref(rtdb, '5_SMART_PARKING/slots');
      
      const unsubscribe = onValue(parkingDataRef, (snapshot) => {
        if (snapshot.exists()) {
      ...
      console.error("Error setting up slots data listener:", err);
      setLoadingSlots(false);
    }
  };
    ...
    <div className="parking-history">
      <div className="history-header">
        ...
        <div className="filter-tabs">
          ...
          <button
            className={`camera-toggle-btn ${showCameraSimulation ? 'active' : ''}`}
            onClick={() => setShowCameraSimulation(!showCameraSimulation)}
          >
            <Camera size={16} />
            {showCameraSimulation ? 'Hide Camera' : 'Show Camera'}
          </button>
        </div>
      </div>
      ...
      {/* Live Camera Simulation System */}
      {showCameraSimulation && (
        <div className="live-camera-system">
          {/* Header Card */}
          <div className="simulation-card header-card">
            <div className="card-icon">
              <Camera size={28} color="#1a73e8" />
            </div>
            <div className="card-content">
              <h2>Live Camera Simulation System</h2>
              <p>Real-time camera streaming</p>
              
              <div className="status-indicator">
                <Check size={18} color="#4CAF50" />
                <span>Live Video Feed + Completion</span>
              </div>
          ...
          {!loadingSlots && (
            <div className="slots-container">
              ...
              <SlotCard 
                slotNumber={1} 
                slotData={slotsData.slot1}
              />
              
              {/* Slot 2 */}
              <SlotCard 
                slotNumber={2} 
                slotData={slotsData.slot2}
              />
              
              {/* Slot 3 */}
              <SlotCard 
                slotNumber={3} 
                slotData={slotsData.slot3}
              />
            </div>
          )}
        </div>
